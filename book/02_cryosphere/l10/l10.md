...
Ice forms in the polar oceans during the polar winter when the atmospheric temperature becomes significantly below zero.  Prior to ice formation, the condition at the ocean surface is that the heat flux from the ocean must equal the heat flux to the atmosphere, the balance of which determines the surface temperature.  Hence

```{math}
F_0 & = & F_A \\
\implies \lambda_0(T_0-T_s) & = & \lambda_A(T_s-T_A)
```

which we can re-arrange to give the surface temperature

```{math}
T_s = \frac{\lambda_0 T_0+ \lambda_A T_A}{\lambda_0 + \lambda_A}
```

Ice begins to form when the surface temperature goes below the melting / freezing temperature $T_s < T_m$, so that

```{math}
T_A < T_m - \frac{\lambda_0}{\lambda_A}(T_0-T_m) < T_m
```
...
Once the ice is growing, we assume that the ice grows in a quasi-steady manner, and hence that there is a linear thermal profile through the ice.  The growth of ice is driven by the heat flux to the atmosphere which balances the diffusive flux of heat across the ice,
...
and is limited by the release of latent heat on solidification and any heat flux from the ocean,

```{math}
:label: 10.stefan_growth
\rho L \dd{h}{t} = k\frac{T_m-T_s}{h} - F_0
```

If we consider a fixed ocean temperature, and hence a fixed oceanic heat flux $F_0 = \lambda_0(T_0-T_m)$, then we can infer (from equation {eq}`10.surf_heat_flux`) that the surface temperature is

```{math}
T_s = \frac{T_m + (\lambda_A h/k)T_A}{1+\lambda_A h/k}
```

So already we can see that the surface temperature approaches the freezing point for thin ice, $T_s \to T_m$ as $h\to0$, while for thick ice it approaches the atmospheric temperature $T_s \to T_a$ as $h\to\infty$.  We can also see that $T_s \simeq T_A$ once $h \ge k/\lambda_A$.

The equation governing the growth of ice (equation {eq}`10.stefan_growth`) can be now be re-written as

```{math}
\rho L \dd{h}{t} = \frac{\lambda_A(T_m-T_A)}{1+\lambda_A h/k} - F_0
```
...
```{math}
h(t \to \infty) = \frac{k}{\lambda_A}\left[\frac{\lambda_A(T_m-T_A)}{F_0} - 1\right]
```
...
A significant aspect of the solidification of sea ice in polar waters is that ocean water is salty, not fresh.  This means that ocean waters, with a salt concentration of approximately $C_0 \simeq 0\ \mathrm{wt\%}$, freeze at a temperature $T_L \simeq -1.8^\circ\mathrm{C}$.  The result is fresh water ice (salt does not get incorporated in the ice lattice) and more salty and cold surface waters.  
