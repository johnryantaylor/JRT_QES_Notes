...
To get a feel for how this model works, we will start by writing the equations to describe the transport of a conservative tracer, $C$, around the model. A conservative tracer is one that is not lost in the system - the total amount stays the same, no matter what processes are acting on it.
