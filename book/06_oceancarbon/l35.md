...
- The ocean is the largest 'reservoir' of immediately-available carbon on Earth's surface.
...
Combined with the mass of the ocean, this tells us that there are ~37,900 PgC dissolved in seawater ({numref}`fig:carbon1box`).
...
Together, these processes act to concentrate carbon in the deep ocean where it cannot exchange with the atmosphere ({numref}`fig:carbonreservoirs`).
...
To understand why we're dividing the ocean this way, let's take a look at the temperature structure of the ocean ({numref}`fig:temp3box`).
