...
Oceanographers have taken a lot of measurements of dissolved carbon in the ocean.
These measurements show that the concentration of Dissolved Inorganic Carbon (DIC) varies widely, with an average 2214 μmol kg<sup>-1</sup>.
...
Carbon in the atmosphere can only exchange with the surface ocean ({numref}`fig:carbon:reservoirs`), which contains much less carbon and is physically and chemically distinct from the average ocean.
...
The influences of the key chemical and biological processes on atmospheric CO<sub>2</sub> are shown in {numref}`fig:carbon:feedbacks`.
...
We will do this by restricting the high- and low-latitude boxes you used in the Stommel model to the surface ocean, and adding a deep ocean reservoir underlying both these boxes.
...
To get a feel for how this model works, we will start by writing the equations to describe the transport of a conservative tracer, $C$, around the model. A conservative tracer is one that is not affected by the processes we are modelling, so that the total amount of the tracer in the system is conserved:
...
Note that neither of these terms influences the deep box because it is not in contact with the atmosphere.
