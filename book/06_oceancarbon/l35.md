...
```{math}
:label: eq:3box:salt
\dv{S_L}{t} &= [\mathrm{transport}] + \frac{E}{V_L} \\
\dv{S_H}{t} &= [\mathrm{transport}] - \frac{E}{V_H} \\
\dv{S_D}{t} &= [\mathrm{transport}] \\
```

where $\mathrm{[transport]}$ refers to the conservative transport terms from {eq}`eq:3box:conservative`.
...
```{math}
:label: eq:3box:heat
\dv{T_L}{t} &= [\mathrm{transport}] - \frac{1}{\tau^{T}_L} (T_L - T_A ) \\
\dv{T_H}{t} &= [\mathrm{transport}] - \frac{1}{\tau^{aT}_H} (T_H - T_A ) \\
\dv{T_D}{t} &= [\mathrm{transport}] \\
```
