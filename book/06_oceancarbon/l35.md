...
There are three main reasons for this, which we will examine in detail in the next three lectures:
...
Together, these processes act to concentrate carbon in the deep ocean where it cannot exchange with the atmosphere ({numref}`fig:carbon:reservoirs`).
...
width: 80%
---
The processes that control the uptake and release of carbon from the ocean, and their influence on atmospheric CO<sub>2</sub>.
```
