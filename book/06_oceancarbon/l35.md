...
- A three-box model is required to understand these patterns.
 ```
...
Each kg of seawater contains ~35 g of dissolved solids, which make the water salty.
You will normally see salinity reported in parts per thousand (ppt) or 'practical salinity units' (PSU), with a typical seawater value somewhere close to 35.
Gasses also exchange between the ocean at the atmosphere which, combined with the relatively large mass of the ocean, make it a significant store of dissolved gasses.
The movement of these gasses around the ocean can have a major impact on atmospheric composition, and particularly CO<sub>2</sub> concentration.
...
Over the past few decades oceanographers have taken a lot of measurements of dissolved carbon in the ocean.
These measurements show that the average concentration of Dissolved Inorganic Carbon (DIC) is around 2214 μmol kg<sup>-1</sup>.
Combined with the mass of the ocean, this tells us that there are ~37,900 PgC dissolved in seawater ({numref}`fig:carbon:1box`).
...
There is significant exchange of carbon between the ocean and atmosphere, with an annual flux of ~90 PgC.
In the modern ocean, there is slightly more carbon uptake than release, making the ocean a net sink for carbon taking up ~1.3 PgC yr<sup>-1</sup>.
...
This means that if carbon in the oceans and atmosphere were in equilibrium, the atmosphere would contain 2214 / 0.03 = **73,800 ppm CO<sub>2</sub>**!
...
Instead, the atmosphere is in equilibrium with the surface ocean ({numref}`fig:carbon:reservoirs`), which is a much smaller carbon reservoir and is physically and chemically distinct from the average ocean.
...
There are three main reasons why the surface ocean deviates from the average ocean, which we will examine in detail in the next three lectures:

1. The ocean has structure - it circulates and is stratified. Only CO<sub>2</sub> in the surface of the ocean can exchange with the atmosphere.
2. CO<sub>2</sub> reacts with water to form non-gaseous dissolved species. The chemistry of carbon in serawater changes how much CO<sub>2</sub> is available to equilibrate with the atmosphere.
3. Biological processes take up carbon from the atmosphere and transport it to the deep ocean, where it cannot equilibrate with the atmosphere.
...
The surface ocean contains a much smaller pool of ~900 PgC which is available for exchange with the atmosphere, only ~0.5% of which is present as CO<sub>2</sub> that can exchange with the atmosphere.
...
The influence of these processes on atmospheric CO<sub>2</sub> is shown in {numref}`fig:carbon:feedbacks`.

```{figure} figures/carbon-feedbacks-ipcc.png
---
name: fig:carbon:feedbacks
figclass: margin-caption
width: 80%
---
The processes that control the uptake and release of carbon from the ocean, and their influence on atmospheric CO<sub>2</sub>. From [IPCC AR4](https://archive.ipcc.ch/publications_and_data/ar4/wg1/en/ch7s7-3-4.html).
```

From the perspective of understanding climate change, the timescales of these storage and release processes is critical.
...
However, to model ocean carbon we must be able to represent the vertical stratitification of the ocean, which then allows us to include processes that move carbon between the surface and deep ocean.
We will do this by adding a deep ocean reservoir underlying the two ocean boxes in the Stommel model.
This provides the necessary vertical and latidudinal structure to represent the ocean carbon cycle.
We will also add an overlying atmosphere model, which will allow us to represent the exchange of carbon between the ocean and atmosphere.
...
To get a feel for how this model works, we will start by writing the equations to describe the transport of a conservative tracer, $C$, around the model:

```{math}
:label: eq:3box_conservative
\dv{C_L}{t} &= \left( q (C_D - C_L) - \frac{V_L}{\tau^{mix}_L} ( C_L - C_D ) \right) / V_L \\
\dv{C_H}{t} &= \left( q (C_L - C_H) - \frac{V_H}{\tau^{mix}_H} ( C_H - C_D ) \right) / V_H \\
\dv{C_D}{t} &= \left( q (C_H - C_D) + \frac{V_L}{\tau^{mix}_L} ( C_L - C_D ) + \frac{V_H}{\tau^{mix}_H} ( C_H - C_D ) \right) / V_D
```

Where the subscripts $H$, $L$, and $D$ refer to the high, low, and deep boxes, respectively.
These equations are very similar to the {ref}`Stommel Model <lecture_32>` equations, but with the addition of a deep ocean box.

Notice a key difference here is that we have to take the volume ($V$) of each of the boxes into account when calculating transport fluxes.

All properties of each box will be influenced by these transport processes, so these equations are the basis of our model.

### 3-box Circulation

To drive circulation in our model we will calculate $q$ from the density gradient between the $L$ and $H$ surface boxes, which is generated by temperature exchange between the ocean and atmosphere and by evaporation.
...
To calculate the transport of salinity ($S$) we add the evaporation term, $E$ to {eq}`eq:3box_conservative` to get:

```{math}
:label: eq:3box_salt
\dv{S_L}{t} &= \left( q (S_D - S_L) - \frac{V_L}{\tau^{mix}_L} ( S_L - S_D ) - E \right) / V_L \\
\dv{S_H}{t} &= \left( q (S_L - S_H) - \frac{V_H}{\tau^{mix}_H} ( S_H - S_D ) + E\right) / V_H \\
\dv{S_D}{t} &= \left( q (S_H - S_D) + \frac{V_L}{\tau^{mix}_L} ( S_L - S_D ) + \frac{V_H}{\tau^{mix}_H} ( S_H - S_D ) \right) / V_D \\
```

To calculate the transport of heat (temperature, $T$), we add terms for the exchange of heat between the ocean and atmosphere to {eq}`eq:3box_conservative` to get:

```{math}
:label: eq:3box_heat
\dv{T_L}{t} &= \left( q (T_D - T_L) - \frac{V_L}{\tau^{mix}_L} ( T_L - T_D ) - \frac{V_L}{\tau^{atex}_T} (T_L - T_A ) \right) / V_L \\
\dv{T_H}{t} &= \left( q (T_L - T_H) - \frac{V_H}{\tau^{mix}_H} ( T_H - T_D ) - \frac{V_H}{\tau^{atex}_T} (T_H - T_A ) \right) / V_H \\
\dv{T_D}{t} &= \left( q (T_H - T_D) + \frac{V_L}{\tau^{mix}_L} ( T_L - T_D ) + \frac{V_H}{\tau^{mix}_H} ( T_H - T_D ) \right) / V_D\\
```

Note that neither of these terms influences the deep box, which is only influenced by the mixing fluxes.
