...
The DIC pool is the primary carbon reservoir in the ocean, and the chemical properties of DIC determine how much CO<sub>2</sub> the ocean can take up.
...
We saw previously that CO<sub>2(g)</sub> behaves like an ideal gas and dissolves from air into water until it reaches an equilibrium with CO<sub>2(aq)</sub> concentration that is described by **Henry's Law** {eq}`eq:carbon:henrys`.
...
A surface currents move from the equator towards the poles (e.g. along Western boundary currents like the Gulf Stream), the water loses heat to the atmosphere and cools down ({numref}`fig:carbon:oceanheat`).
...
The drop in temperature causes $K_0$ to increase, allowing the water to take up more dissolved CO<sub>2</sub> at a given atmospheric pCO<sub>2</sub> ({numref}`fig:carbon:ocean-atmos`).
...
Consequently, upwelling zones bring CO<sub>2</sub>-rich water to the surface, which is released to the atmosphere as the water warms and $K_0$ decreases ({numref}`fig:carbon:ocean-atmos`).
...
width: 100%
---
The disequilibrium between surface ocean and atmospheric pCO<sub>2</sub>. Notice that the ocean tends to take up CO<sub>2</sub> along the path or poleward surface currents (blue colours; e.g. North Atlantic), and release CO<sub>2</sub> in upwelling zones where carbon-rich water is brought to the surface and heated up (red colours; e.g. at the equator and in the North Pacific). The first-order patterns can be explained by the solubility pump, but the magnitude of the change is much larger than predicted by Henry's Law alone.
```
...
For a typical surface ocean DIC of ~2000 μmol kg<sup>-1</sup>, this corresponds to a CO<sub>2</sub> concentration of ~10 μmol kg<sup>-1</sup>, which is within range of the [CO<sub>2(aq)</sub>] concentration predicted by Henry's Law (8-34 μmol kg<sup>-1</sup>)!

The reaction of CO<sub>2(aq)</sub> with water explains why the concentration of DIC in seawater is so high.

### DIC Speciation

The speciation of DIC shown in {numref}`fig:carbon:bjerrum` is described by acid-base equilibria described by the **equilibrium constants** $K_1$ and $K_2$:
...
These can be combined with the definition of DIC {eq}`eq:carbon:dic` to calculate the concentration of each species as a function of [DIC], pH and the equilibrium constants $K_1$ and $K_2$:
...
Because these Ks are sensitive temperature, salinty and pressure, the speciation of DIC and pH also shift as the physical and chemical properties of the water change.
...
The relationship between pCO<sub>2</sub> and DIC is not straightforward.
Initially, increasing pCO<sub>2</sub> increases the concentration of [CO<sub>2</sub>\*] because these two quantities are linked by Henry's Law.
...
However, this amplifying effect of DIC speciation is not uniform.
The new equilibrium state at higher pCO<sub>2</sub> also has a reduced pH, which alters the balance of DIC species to favour [CO<sub>2</sub>\*].
...
For an initial pCO<sub>2</sub> of 400 ppm with N<sub>air</sub> of 1.773x10<sup>20</sup>, this yields a total mass of ~850 GtC in out model atmosphere.
...
Note that we have to be a bit careful with units here: $K_0 pCO_2$ will return units of $\mu mol~kg^{-1}$ if $pCO_2$ is given in ppm - this has to be multiplied by density (~1025 kg m<sup>-3</sup>) to convert to a volume concentration ($\mu mol~m^{-3}$), and multiplied by a factor of $10^-6$ to convert to units of $\mathrm{mol~m^{-3}}$ to match $[CO_2]_i$.
...
We can track the transport of DIC just like any other conservative tracer {eq}`eq:3box:conservative`, but keeping track of speciation is a bit more complicated...
...
For our purposes, it is sufficient to know that TA is conservative, and that TA and DIC together can be used to calculate aqueous carbon speciation ({numref}`fig:carbon:DIC-TA`).
...
TA is a difficult concept that is commonly misunderstood. It is *not* the opposite of acidity. For a chemist, TA is "*a measure of the excess bases (proton acceptors) over acids (proton donors), and is operationally defined by the titration with H<sup>+</sup> of all weak bases present in the solution*" (Sarmiento & Gruber, 2006). In practical terms, it is the moles of H<sup>+</sup> you must add to a kg of seawater to reach the equivalence point where [H<sup>+</sup>] = [HCO<sub>3</sub><sup>-</sup>]. For a geochemist, "*TA is equal to the charge difference between conservative cations and anions*", so is a measure of the charge imbalance of seawater. It can also be thought of as the 'buffering capacity' of water, as higher alkalinity water has more proton acceptors so can better resist changes in pH.

It is approximately defined as:
$
