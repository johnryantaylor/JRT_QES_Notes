...
```{highlights}
Key Points:
- The solubility of CO<sub>2</sub> in water is temperature and salinity dependent.
- Variability in solubility interacts with ocean circulation to drive the 'solubility pump'.
- The solubility pump traps high-CO<sub>2</sub> water in the deep ocean.
- CO<sub>2</sub> reacts with water to create non-exchangeable DIC species.
- These reactions increase the carbon storage capacity of ocean water.
- The speciation of DIC is pH dependent, which is critical for understanding the ocean carbon budget.
 ```
...
Note that at normal surface seawater pH (~8.1, {numref}`fig:carbon:bjerrum`), only ~0.5% of the DIC is in the form of CO<sub>2(aq)</sub>. 
Most DIC exists as the non-exchangeable species bicarbonate and carbonate.

### DIC Speciation

The speciation of DIC shown in {numref}`fig:carbon:bjerrum` is described by acid-base equilibria described by the **equilibrium constants** K<sub>1</sub> and K<sub>2</sub>:
...
\dv{DIC_{CO2,i}}{t} = - \frac{1}{\tau^{CO2}_i} V_i \left([CO_2]_i - \underbrace{K_{0,i} ~ pCO_2}_{\mathrm{\mu mol~kg^{-1}}} \right)
```
...
\dv{pCO_2}{t} = \sum_{i=L,H} \left(\frac{1}{\tau^{CO2}_i} V_i \left([CO_2]_i - K_{0,i} ~ pCO_2 \right) \right) \times 10^{6} / N_{air}
``` 
...
As no processes in our model currently modify TA (they will soon), we can model TA using the equations for conservative tracer transport given in {eq}`eq:3box:conservative`.
...
\dv{DIC_L}{t} &= [\mathrm{transport}] - \frac{1}{\tau^{CO2}_L} ([CO_2]_L - K_{0,L} ~ pCO_2 ) \\
\dv{DIC_H}{t} &= [\mathrm{transport}] - \frac{1}{\tau^{CO2}_H} ([CO_2]_H - K_{0,H} ~ pCO_2 ) \\
