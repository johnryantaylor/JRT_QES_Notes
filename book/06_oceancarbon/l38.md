...
Three groups of marine plankton are responsible the majority of calcification in the open ocean:

- **Coccolithophores** are microscopic (3-50 μm) phytoplankton which produce intricate calcium carbonate plates called coccoliths. Coccolithophores are a major calcifyer in the open ocean, and are coccoliths are the main constituent of chalk found in uplifted marine sediments (e.g. the White Cliffs of Dover). The exact purpose of these coccoliths is unclear, although hypotheses include buoyancy control, protection from grazing and light scattering.
- **Foraminifera** are slightly larger (50-500 μm) zooplankton which produce calcium carbonate shells called tests. These tests have a distinctive multi-chambered structure, which grows gradually throughout the organism's life cycle.
- **Pteropods** are larger-still (0.5-10 mm) zooplankton who's name literally translates to 'wing foot'. Pteropods are gastropods, which (mostly) produce a recognisable calcium carbonate 'snail shell' structure, with the soft parts of the body adapted into 'wing' structure that allow them some limited movement through the water.
...
Coccoliithophores and foraminifera produce calcite, while pteropods produce aragonite.

Just like organic matter, these shells sink from the surface when the organisms die, exporting the CO<sub>3</sub><sup>2-</sup> from the surface to the deep ocean.
The export of CO<sub>3</sub><sup>2-</sup> removes DIC from the surface ocean but, because taking up CO<sub>3</sub><sup>2-</sup> alters the speciation of the DIC that remains in the surface ocean, surface pH is reduced and the fraction of DIC that exists as CO<sub>2</sub> increases.
This causes CO<sub>2</sub> to be *released* to the atmosphere when CO<sub>3</sub><sup>2-</sup> is removed from the surface ocean.

Removing CO<sub>3</sub><sup>2-</sup> causes pH to drop because a HCO<sub>3</sub><sup>-</sup> dissociates to replace it, releasing H<sup>+</sup> into the solution.
This causes the fraction of DIC that exists as CO<sub>2</sub> to increase (the equilibrium shifts to the left in {numref}`fig:carbon:bjerrum`).
Calcification is therefore an immediate *source* of CO<sub>2</sub> to the atmosphere because if its effect on DIC speciation, even though it is also exporting DIC to the deep ocean.
...
The CaCO<sub>3</sub> minerals become more soluble at higher pressure, and $K_{sp}$ increases with depth.
...
The effects of pressure on solubility and speciation then combine with the biological pump to make CaCO<sub>3</sub> dissolution occure at shallower depths than would be expected from pressure alone.
...
Another layer of complexity to this story is the effect of particle aggregates.
...
The respiration of bacteria in these aggregates can drive very high local CO<sub>2</sub> concentrations, dramatically reducing pH and CO<sub>3</sub><sup>2-</sup> and creating a highly undersaturated environment.
...
The carbonate pump acts to concentrate DIC and TA in the deep ocean in a 1:2 ratio, following our definitions of DIC and TA:
...
The carbonate pump and is inextricably connected to the biological pump, because CaCO<sub>3</sub> production is driven by biological productivity.
...
Seawater has a density of ~1025 kg m<sup>-3</sup>, organic matter has a typical density of ~1030-1500 kg m<sup>-3</sup>, and calcite has a density of 2710 kg m<sup>-3</sup>.
...
This feedback is known as **ballasting**.
...
This would reduce [CO<sub>3</sub><sup>2-</sup>], and therefore the saturation state of the surface ocean.
Reducing the saturation state has the potential to reduce CaCO<sub>3</sub> production, by marine organisms, and enhance CaCO<sub>3</sub> dissolution at shallow depths.
...
However, when the effect of CaCO<sub>3</sub> production on ballasting the biological pump is taken into account, the effect of ocean acidification on the carbonate pump is less clear.
This is a significant source of uncertainty in our predictions of the uptake of CO<sub>2</sub> by the ocean in future.

```{figure} figures/carbon-Omega-pCO2.png
---
name: fig:carbon:omega-pco2
figclass: margin-caption
width: 70%
---
...
This term modifies both DIC and TA in our model in a 1:2 ratio {eq}`eq:carbon:carbonate-pump-dic-ta`, so DIC is modified following:
...
where the terms in square brackets represents the transport {eq}`eq:3box:conservative`, gas exchange {eq}`eq:3box:dic:solubility` and biological pump {eq}`eq:3box:dic:biopump` terms.  TA is modified following:
