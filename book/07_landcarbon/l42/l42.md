...
Knowing the methane concentration is half the battle, the other is figuring out where it came from.
...
We can also use measurements and models to try to understand the amount of $\ce{OH-}$ radical in the atmosphere, which is the dominant sink for methane (~90%).
Reaction rates of methane oxidation are strongly linked to local changes in $\ce{OH-}$ concentration.
Two approaches derive estimates of OH quantity in the atmosphere:
...
Both approaches derive a 10-15% uncertainty on global OH mean concentrations.
...
This may set off a positive feedback loop.
